...
public class SessionFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        try {
            HttpSession session = wrappedRequest.getSession(false);// don't create if it doesn't exist

            if(session != null) {
                //wrappedRequest.setAttribute("name", session.getAttribute("user"));
                chain.doFilter(wrappedRequest, wrappedResponse);
            } else {
                request.setAttribute("Error", "Session expired! Login please");
                wrappedRequest.getRequestDispatcher("login.jsp").forward(wrappedRequest, wrappedResponse);
