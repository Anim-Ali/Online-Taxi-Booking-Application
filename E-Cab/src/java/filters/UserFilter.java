...
public class UserFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        List<String> adminServlets = Arrays.asList( "AdminServlet", "AssignDriverServlet", "DriverManagerServlet", 
                                        "ReAssignDriverServlet", "adminReports");
        List<String> customerServlets = Arrays.asList( "BookingConfirmServlet", "CustomerServlet", "customerReports");
        List<String> driverServlets = Arrays.asList( "DriverServlet", "driverReports");
        ...
        try {
            ...
            if(session != null){
                ...
                switch (type) {
                    case "admin":
                        if (adminServlets.contains(servletName)) {
                            chain.doFilter(request, response);
                        }
                        else {
                            request.setAttribute("Error", "This feature is not available to this user. Please login as the right user");
                            request.getRequestDispatcher("login.jsp").forward(request, response);
                        }
                        break;
                    
                    case "customer":
                        if (customerServlets.contains(servletName)) {
                            chain.doFilter(request, response);
                        }
                        else {
                            request.setAttribute("Error", "This feature is not available to this user. Please login as the right user");
                    ...
                    case "driver":
                        if (driverServlets.contains(servletName)) {
                            chain.doFilter(request, response);
                        }
                        else {
                            request.setAttribute("Error", "This feature is not available to this user. Please login as the right user");
